...
document.addEventListener("DOMContentLoaded", function () {
          ...
          if (docId) {
            ...
            firestoreService
              ...
              .then((snapshot) => {
                ...
                output(
                  `<b>Getting <i>${docId}</i> from <i>${collection_name}</i> </b> <br /> <b>Response</b>: <br /> ${escapeHtml(
                    JSON.stringify(data)
                  )}`
                );
              })
  ...
  cloudfuncForm.addEventListener("submit", (e) => {
    ...
    if (funcParams[funcParams.length - 1] != ")") {
      M.toast({
        html: `Please enter a valid invoke syntax. <br />Reason: The input is not ending with  ')'`,
      });
      return;
    }
    ...
    try {
      ...
      M.toast({
        html: "Invalid invoke syntax. For more information, open the browser's console.",
      });
      console.log(e);
    }
  });
